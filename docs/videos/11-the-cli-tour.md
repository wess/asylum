...
**Duration:** ~6 min · **Level:** Advanced
**You'll learn:** every `asylum` subcommand at a glance — worktree, run, search, control, wait, plugin, layout, and the computer-use trio.
...
- Keep each subcommand beat tight — this is a reference tour, not a deep dive. The control/wait deep dive is episode 12.

## Recap card (end screen)
- Subcommands: `worktree`, `run`, `search`, `control`, `wait`, `plugin`, `layout`, plus computer-use `snapshot`/`click`/`fill`.
