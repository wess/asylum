# Build Asylum (release) for Windows and produce a portable .zip and an MSI
# installer under dist/windows. Mirrors packaging/linux.sh. Intended for the
# windows-latest runner; run locally with PowerShell 7+ (pwsh).
#
# The binary is the `asylumdev` bin from crates/app, shipped as `asylum.exe`. The
...
#
# Usage: pwsh packaging/windows.ps1 [-Arch x86_64]
param(
...
Write-Host "[windows] Asylum $version for $triple"
...
$bin = "target\$triple\release\asylumdev.exe"

# --- staging tree (shared by the zip and the MSI harvest) ------------------
$stem = "asylum-$version-windows-$Arch"
$stage = Join-Path $out $stem
New-Item -ItemType Directory -Force -Path $stage | Out-Null
Copy-Item $bin (Join-Path $stage "asylum.exe")
...
try {
    ...
    wix build "packaging\windows\asylum.wxs" `
