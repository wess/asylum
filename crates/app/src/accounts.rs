            ...
            div()
                ...
                .child(
                    div()
                        ...
                        .child(Text::new(SharedString::from(row.account.label.clone())).size(Size::Sm).dimmed()),
                )
                .child(if active {
                    Badge::new("active").color(ColorName::Green).variant(Variant::Light)
                } else {
                    Badge::new("switch").color(ColorName::Gray).variant(Variant::Light)
                }),
        );

    if let Some(usage) = &row.usage {
        let pct = usage.fraction().map(|f| (f * 100.0) as u32).unwrap_or(0);
        let limit = usage.limit.map(|l| l.to_string()).unwrap_or_else(|| "∞".into());
        card = card.child(
    ...
    let clickable = div()
        ...
        .on_click(move |_, _, cx| {
            handle.update(cx, |root, cx| {
                let _ = root.db.activate_account(id);
                cx.notify();
            });
        })
